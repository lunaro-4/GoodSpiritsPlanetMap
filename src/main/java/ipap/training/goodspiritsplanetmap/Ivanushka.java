...
//        System.out.println(Arrays.toString(costToNode));
    ...
    public static HashMap[] getDict(Layer[] nodeMap) {
        ...
        HashMap<Integer,String > matrIndexToName = new HashMap<>();
        int counter = 0;
        for (int i = 0; i < newMap.length; i++) {
            for (int j = 0; j < newMap[i].nodeList.length; j++) {
                ...
                matrIndexToName.put(counter,s);
            }
        }
        return new HashMap[] {nameToMatrIndex,matrIndexToName};
    }


    public static int[][] convert2(Layer[] nodeMap){
        //  converts nodeMap to adjacency matrix
...
//        System.out.println("\t"+s.curPath+"  "+nodeIndex);
    ...
    public static Layer[] readFromFile(String filename)throws IOException{
